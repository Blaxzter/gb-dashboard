...
export default {
    ...
    computed: {
        ...
        missingTexts() {
            const used_texts = _.flatMap(this.store.gesangbuchlieder, (lied) => [lied.textId]);
            return _.filter(this.store.texts, (text) => used_texts.indexOf(text.id) === -1);
        },
        missingMelodies() {
        ...
        missingFiles() {
            ...
            return _.filter(this.store.files, (file) => used_files.indexOf(file.id) === -1);
        },
        missingAuthors() {
...
<template>
    ...
    <div class="text-body-1">Gefundene Einträge: {{ selectedElements.length }}</div>
    <v-list dense>
        <v-list-item
