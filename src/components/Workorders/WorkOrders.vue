<template>
    ...
    >
        <div class="text-h4 mb-4 text-no-wrap">{{ filter_auftraege.length }} Arbeitsaufträge</div>
        ...
        <v-card-text>
            <v-window v-model="tab">
                <v-window-item value="alle">
                    <AuftragCards v-if="display_style === 0" :auftraege="filter_auftraege" />
                    <AuftragTable v-else-if="display_style === 1" :auftraege="filter_auftraege" />
                </v-window-item>
                <v-window-item v-for="(label, index) in current_tabs" :key="index" :value="index">
                    <AuftragCards v-if="display_style === 0" :auftraege="current_group[label]" />
                    <AuftragTable
            ...
            if ('sonstiges' in auftragsartMelodie)
                status.push(
                    ..._.filter(
                        ...
                        (elem) => _.find(status, (elem_c) => elem_c.id === elem.id) === undefined,
                    ),
                );
