<template>
  <v-card :loading="loading" title="Gesangbuchlied wird Hochgelanden">
    <v-card-text>
      ...
      <v-card
        title="Gesangbuchlied"
        v-if="gesangbuchlied"
        class="mb-4"
      >
        <v-card-text>
          <div>
            <strong>ID: </strong>{{ gesangbuchlied.id }}
            <strong>Titel: </strong>{{ gesangbuchlied.titel }}
          </div>
            ...
            >
              <v-card-text>
                <div>
                  ...
                  <div><strong>Gesangbuchlied: </strong> {{ elem.gesangbuchlied_id }}</div>
        ...
        <v-card-text class="mb-0">
          <div>
            <div>
              ...
              <strong>Text Titel: </strong>{{ text.titel }}
            </div>
            ...
            <div class="d-flex flex-row flex-wrap">
              <v-card
                width="200"
                class="me-5 mb-5"
                v-for="(elem, index) of text_author_mapping" :key="index"
      ...
      >
        <v-card-text>
          <div class="mb-5">
            <strong>ID: </strong>{{ melodie.id }}
            <strong>Titel: </strong>{{ melodie.titel }}
          </div>
          ...
          <div class="d-flex flex-row flex-wrap">
            <v-card
              ...
              class="me-5 mb-5"
              v-for="(elem, index) of melodie_author_mapping" :key="index"
            >
              <v-card-text>
                <div>
                  <div><strong>Melodie</strong> Author {{ index + 1 }}:</div>
          ...
          <div class="d-flex flex-row">
            <v-card
              ...
              class="me-5 mb-5"
              v-for="(elem, index) of melodie_files" :key="index"
            >
              <v-card-text>
                <div>
    ...
    <v-card-actions>
      <v-btn block color="primary" variant="tonal" @click="$emit('reset')">Neues Lied anlegen</v-btn>
    </v-card-actions>
  </v-card>
</template>
